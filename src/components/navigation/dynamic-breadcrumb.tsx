...
export default function DynamicBreadcrumb(): JSX.Element | null {
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    if (isSmallScreen) {
      ...
      items.push(
        <BreadcrumbSeparator
          key={`sep-${currentHref}`}
          className="ml-1 dark:text-secondary"
        >
          <TbSlashes />
        </BreadcrumbSeparator>
      );
      ...
      if (pathSegments.length > 2) {
        ...
        items.push(
          <BreadcrumbSeparator key="sep-2" className="ml-1 dark:text-secondary">
            <TbSlashes />
          </BreadcrumbSeparator>
        );
      ...
      if (pathSegments.length > 0) {
        ...
        const lastSegment = capitalizedSegments[capitalizedSegments.length - 1];
        items.push(
          <BreadcrumbSeparator key="sep-1" className="ml-1 dark:text-secondary">
            <TbSlashes />
          </BreadcrumbSeparator>
        );
      ...
      pathSegments.forEach((_, index) => {
        ...
        items.push(
          <BreadcrumbSeparator
            key={`sep-${href}`}
            className="mx-4 dark:text-secondary"
          >
            <TbSlashes />
          </BreadcrumbSeparator>
        );
  ...
  return (
    <div className="mx-auto w-10/12 md:w-11/12">
      ...
      >
        <ul className="flex flex-row items-center gap-2 font-[BarlowCondensed]">
          {breadcrumbItems}
        </ul>
      </nav>
    </div>
